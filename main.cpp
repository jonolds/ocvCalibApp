...
"{t        | charuco | Template for calibration (circles, chessboard, dualCircles, charuco) }"
"{sz       | 16.3    | Distance between two nearest centers of circles or squares on calibration board}"
"{dst      | 295     | Distance between white and black parts of daulCircles template}"
"{w        | 6        | Width of template (in corners or circles)}"
"{h        | 8        | Height of template (in corners or circles)}"
...
int main(int argc, char** argv) {
	...
	cv::Ptr<FrameProcessor> showProcessor;
	cv::Ptr<FrameProcessor> capProcessor = cv::Ptr<FrameProcessor>(new CalibProcessor(globalData, capParams));
